...
namespace IsYonetimPro.Services.Concrete
{
    public class EmployeeManager : IEmployeeService
    {
        ...
        public async Task<IResult> AddAsync(EmployeeAddDto employeeAddDto, int createdById)
        {
            var employee = _mapper.Map<Employee>(employeeAddDto);
            employee.CreatedById = createdById;//dto da almadığımız için burada ekledik.

            await _unitOfWork.Employee.AddAsync(employee);//veritabanına gönderdik.
            await _unitOfWork.SaveAsync();
            return new Result(ResultStatus.Success, $"{employeeAddDto.EmployeeName} isimli çalışan sisteme kaydedildi.");
        }
